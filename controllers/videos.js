...
function setupListViewState(req) {
  ...
  viewState.start =
    ...
    (viewState.pageNumber - 1) * LIST_PAGE_SIZE;
...
function setupListViewArguments(req, viewState, result) {
  return Object.assign(libMetadata.fromRequest(req), {
    ...
    previousPageNumber:
      viewState.pageNumber === 2 ? false : viewState.pageNumber - 1,
    currentPageNumber: viewState.pageNumber,
    sortOrder:
      viewState.sortOrder === DEFAULT_SORT_ORDER ? false : viewState.sortOrder,
...
async function listPwas(req, res, next, sortOrder, filters) {
  ...
  console.log('hello');
  const html = await render(res, 'videos/list.hbs', {});
  res.send(html);
  // pwaLib
  ...
  //       res,
  //       'pwas/list.hbs',
  //       setupListViewArguments(req, viewState, result),
  //     ),
  //   )
...
//   const viewState = setupListViewState(req);
//   libPwaIndex
//     .searchPwas(viewState.searchQuery)
//     .then((result) =>
//       render(
//         res,
//         'pwas/list.hbs',
...
//   return pwaLib.find(pwaId).then((pwa) => {
//     return lighthouseLib.findByPwaId(pwaId).then((lighthouse) => {
//       if (
//         lighthouse &&
//         lighthouse.lighthouseInfo &&
//         Object.prototype.toString.call(lighthouse.lighthouseInfo) ===
//           '[object String]'
//       ) {
